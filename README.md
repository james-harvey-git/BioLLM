...
- If `benchmark.task_field` is set (recommended), task boundaries follow that explicit field.
- If `benchmark.task_field` is unset, tasks are synthetic hash buckets (`sha256(prompt) % num_tasks`), which are useful for infrastructure checks but are not semantic task families.
...
- Wake batch size: `4` (pilot stability default)
- Seeds: `7 42 123`
...
- `pseudo_source_mode`
  - `mixed`: pseudo prompts combine replay-echo prompts and random token prompts.
  - `replay_echo`: pseudo prompts come only from replay inputs.
  - `random_tokens`: pseudo prompts are random token sequences (outside replay).
...
- HF mid-layer injection:
  ...
  - If HF gradient checkpointing is enabled, the adapter temporarily disables it during `forward_from_injection` to avoid hook/recompute ambiguity in backward passes, then restores it immediately.

- Pseudo-rehearsal:
  - During sleep, pseudo examples are generated from configurable sources (`pseudo_source`): replay-echo prompts, random-token prompts, or a mixture.
  - Pseudo teacher logits are produced by the current full system (`base + hippocampus`).
...
- Fisher capability-mix:
  ...
  - When Fisher uses the capability mix, it reuses the existing distillation forward pass instead of running a second base-model forward.
